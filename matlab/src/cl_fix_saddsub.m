...
%--------------------------------------------------------------------------
%
% Conditionally add or subtract two fix-point numbers/vectors.
% Simple implementation (optimized for performance and resource usage
% within the FPGA).
%
% NOTE: This function leads to an error of one LSB if subtraction is
%       selected.
%
...
%
function result = cl_fix_saddsub (a, a_fmt, b, b_fmt, add, result_fmt, round, saturate)

temp_fmt = cl_fix_format (a_fmt.Signed || b_fmt.Signed, max (a_fmt.IntBits, b_fmt.IntBits)+1, max (a_fmt.FracBits, b_fmt.FracBits));
temp = a+(-1).^(add==0).*b - (add==0).*2^-temp_fmt.FracBits;
